  ...
  const [selectedBook, setSelectedBook] = useState({ id: '', name: '', author: '' });
   const [newBook, setNewBook] = useState({ name: '', author: '' });
      ...
      const fetchBooks = async () => {
        try {
          const response = await axios.get('https://week15-defficharlina.cyclic.app/api/v1/books');
...
useEffect(() => {
  ...
  const handleEditSubmit = async () => {
    try {
      await axios.put(`https://week15-defficharlina.cyclic.app/api/v1/books/${selectedBook.id}`, {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      await axios.delete(`https://week15-defficharlina.cyclic.app/api/v1/books/${id}`);
  ...
  const handleAddBook = async () => {
    try {
      await axios.post('https://week15-defficharlina.cyclic.app/api/v1/books', {
        name: newBook.name, 
